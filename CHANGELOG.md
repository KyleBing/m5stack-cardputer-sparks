...
- **空调自动化（`n` / AC AUTO）**：可选 BLE 温湿度计；温度 `> on` 连续 filter 次开空调、`< off` 连续 filter 次关空调（默认 29/26/3）；进 App 15s 倒计时后息屏自动工作，任意键亮屏；展示开关机次数、当前温湿度与 12h 曲线（每分钟一点）
- **配置入口**：App 内 `c` 配置页；Options 侧栏 `ac auto`；Config Web 顶栏「空调自动化」(`/ac-auto`)；参数含传感器、阈值、过滤次数、品牌/模式/设定温度/风力
