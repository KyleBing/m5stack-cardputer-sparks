...
进入方式：主菜单 `e` → [Grove](./ex-i2c) `1` / `r`

左侧 Grove 上的 FM 收音机。买现成 **TEA5767** 或 **RDA5807M** 四针模块即可（I2C `0x60` 或 `0x10`/`0x11`），共用一套界面。声音从**模块耳机孔**输出。芯片接线与驱动 API 见 [Grove](./ex-i2c)。
...
主界面无操作约 **30s** 后自动息屏（续播）；Help / 列表 / Tuner / 搜台中不自动灭屏。
