# Dice

Open with main menu `g`, then Games `4`.

Warm ivory dice with tabletop physics. Shake the device or hold Space to charge a toss; IMU shake strength and Space hold time directly control motion amplitude, then each die quickly eases into its face-up pose.

## Screenshots

<div class="shot-row">

![dice-rolling](/shots/app_games_dice_rolling.png)
![dice-result](/shots/app_games_dice_result.png)

</div>

## Shortcuts

| Key | Action |
|-----|--------|
| `-` `=` | Dice count − / + (1–5) |
| Hold `Space` / `GO` | Charge, then toss on release (full power at 1.8 s) |
| Shake device | Toss (rising-edge trigger) |
| `h` | Help |

## Notes

- Dice roll, bounce, and collide across a full-screen green felt tabletop.
- While `Space` or the side `GO` button is held, a bottom `POWER` bar shows the live 0–100% charge.
- Gentle shakes create small disturbances; hard shakes continuously add stronger translation, bounce, and spin impulses.
- Final poses use fast, smooth easing instead of snapping. A dynamic gold banner reveals the total (`TOTAL`).
- About 1 s later, dice line up left-to-right by their settled screen positions, with each value shown underneath.
- The enlarged dice use a prominent red 3 px pip for one and consistent 2 px black pips for all other values.
